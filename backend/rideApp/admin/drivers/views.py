...
class CreateDriver(APIView):
    def post(self, request):
        ...
        if status_request not in VALID_STATUS_CHOICES:
            return Response(
                {"error2": "Invalid status value."},
                status=status.HTTP_400_BAD_REQUEST
            )

        driver_serializer = DriverSerializer(data=request.data)
        if driver_serializer.is_valid():
            # Set the is_createby field with the user who created the driver
            custom_user = getattr(request, 'user_id', None)
            if custom_user:
                driver_serializer.validated_data['is_createby'] = custom_user
...
class EditDriver(APIView):
    def put(self, request, driver_id):
        ...
        if serializer.is_valid(raise_exception=True):
            # Set the is_updateby field with the user who updated the driver
            custom_user = getattr(request, 'user_id', None)
            if custom_user:
